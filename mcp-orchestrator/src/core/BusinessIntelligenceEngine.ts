...
export class BusinessIntelligenceEngine {
    ...
    private analyzeSeasonalTrends(bookings: BookingPattern[]) {
        ...
        return {
            hasOpportunity: true,
            peakSeason: highestSeason ? highestSeason[0] : 'general',
            peakDemand: highestSeason ? highestSeason[1] : 0,
            confidence: 76
        };
    }
    ...
    private analyzeCapacityUtilization(bookings: BookingPattern[]) {
        ...
        const totalBookings = largeBookings + smallBookings;
        if (totalBookings === 0) return { needsAdjustment: false, largeUtilization: 0, recommendation: 'none', confidence: 0 };

        const largeUtilization = largeBookings / totalBookings;
